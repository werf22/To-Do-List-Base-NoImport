    ...
    switch (fieldConfig.type) {
      case 'multi-select':
        ...
        if (Array.isArray(fieldConfig.options)) {
          // First, check for the semicolon format (our standard export format)
          if (value.includes(';')) {
            return value.split(';').map(item => item.trim()).filter(Boolean);
          }
          // Check for special pipe-separated format (backward compatibility)
          else if (value.startsWith('"[[') && value.endsWith(']]"')) {
            // Extract the content inside the double brackets and quotes
            const content = value.slice(3, -3);
            // Split by pipe and restore any encoded pipes
            return content.split('|').map(item => item.trim().replace('&#124;', '|')).filter(Boolean);
          }
          ...
          // Handle quoted values with commas
            ...
            return splitCsvWithQuotes(value);
          }
          else {
        ...
        return value ? [value] : []; // Default to single-item array if non-empty
